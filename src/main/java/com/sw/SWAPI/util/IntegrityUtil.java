...
public class IntegrityUtil {
	...
	public String dealData(List<JSONObject> listData){
		...
		if ("add".equals(action_Type)) {//
			...
			if (docList == null || docList.isEmpty() || docList.size() == 0) {
				...
				for (Map<String, String> docInfo : docList) {
					if (project.equals(docInfo.get("Project"))) {// 如果目标项目已经存在数据，不创建
						log.error("206 - Document has created in project: [" + project + "]!");
			...
			try {
				...
				log.error("查询数据：" + APIExceptionUtil.getMsg(e));
			}
			if (docList == null || docList.isEmpty()) {// 更新判断文档是否存在
				log.error("204 - Document hadn't create。Please check you action type!");
				...
				}else if (!Constants.DOC_INIT_STATE.equals(curState)) {// 文档状态判断
					log.error("205 - Document now is in reivew or published, can not update!");
